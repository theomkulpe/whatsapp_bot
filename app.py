...
os.environ["OPENAI_API_KEY"] = "sk-proj-vM4HWzCFJgSP5buK43W4T3BlbkFJ6ZJhwymGN2UXuupN5Sgb"
os.environ["EMBEDDING_MODEL"] = "text-embedding-ada-002"
os.environ["PPLX_MODEL"] = "llama-3-sonar-large-32k-online"
...
def pplxity(prompt):
    payload = {
        ...
        "messages": [
            {
                "role": "system",
                "content": '''Precisely give the final answer in less words. Do not give incomplete answer.'''
                              # Example what is the stock price of '''
            },
            {
                "role": "user",
                "content": prompt
...
def newChat():
    ...
    with open('prePrompt.txt') as file:
...
'''with  gr.Blocks() as iface:
  input=gr.Textbox(lines=2, placeholder="Type your question here...",label = "Question")
  instructions = gr.Textbox(lines=2, placeholder="Additional Instructions if any...",label = "Instructions")
  btn1 = gr.Button(value="Submit")
  output1 = gr.Textbox(label="Chhirag-GPT Output")
  output2 = gr.Textbox(label="chatgpt Output")
  btn1.click(chat_with_llm,inputs = [input,instructions],outputs=[input,output1,output2])
    
  btn2 = gr.Button(value="Press to start a New Chat")
